...
//? 1: $and: Performs a logical AND operation on an array of expression, where all expressions must be true for the document to match.

//! Find products with price greater than 100 and name equal to "Diamond Ring"

// db.products.find({ $and: [ { 'price': { $gt: 10 } }, { 'name': 'Notebook Collections' } ] })
...
//? 2: $or: Performs a logical OR operations on an array of expressions, where at least one expression must be true for the document to match.

// We can use logical operator only when we have duplicate fields
// db.products.find({'price': 129, 'price': 39})
// but we can write the same in $or operator
// db.products.find({$or: [{'price': 129, 'price': 39}]})

//? 3: $nor: Performs a logical NOR operation on an array of expressions, where none of the expressions must be true for the document to match
...
//? 4: $not: Performs a logical NOT operation on the specified expression, inverting the result.
